...
fixtures = [
	...
	# Desktop Icon and Workspace Sidebar are both only ever auto-generated by
	# Frappe once, during a *fresh* `bench install-app` - a teammate who
	# already had chw installed and just pulls these commits + runs `bench
	# migrate` never gets that auto-generation step re-run, so without these
	# as fixtures, "CHW App" silently stays invisible on /desk for them even
